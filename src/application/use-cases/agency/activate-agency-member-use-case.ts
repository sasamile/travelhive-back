...
export interface ActivateAgencyMemberInput {
  agencyId: bigint;
  memberId: bigint;
  userId: string; // ID del admin que está activando
}

@Injectable()
export class ActivateAgencyMemberUseCase {
  constructor(
  ...
  async execute(input: ActivateAgencyMemberInput) {
    ...
    // Verificar que el usuario que activa es admin de la agencia
    ...
    if (adminMembership.role !== 'admin') {
      throw new ForbiddenException('Solo los administradores pueden activar miembros');
    }
    ...
    // Activar el miembro
    const updatedMember = await this.agencyMemberRepository.toggleActive(input.memberId, true);
